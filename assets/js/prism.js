/* PrismJS 1.28.0
https://prismjs.com/download.html#themes=prism-dark&languages=markup+css+clike+javascript+apacheconf+aspnet+autohotkey+bash+batch+c+csharp+cpp+cmake+coffeescript+css-extras+csv+dart+diff+dns-zone-file+elixir+etlua+erlang+gdscript+git+glsl+go+handlebars+haskell+hlsl+http+hpkp+hsts+ignore+ini+java+javadoc+javadoclike+jsdoc+js-extras+json+json5+jsonp+latex+less+lisp+lua+makefile+markdown+markup-templating+nasm+nginx+perl+php+phpdoc+php-extras+powershell+promql+python+jsx+tsx+regex+rest+ruby+rust+sass+scss+shell-session+solution-file+sql+stylus+swift+systemd+toml+twig+typescript+typoscript+verilog+vhdl+vim+wasm+xml-doc+yaml&plugins=autolinker+show-language+toolbar+copy-to-clipboard+match-braces */
